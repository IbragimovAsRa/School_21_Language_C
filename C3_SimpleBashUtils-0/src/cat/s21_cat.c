...
config command_line_dispatcher(char **argv);

int next_row_is_empty(
    FILE *fp); // возвращает 1 если строка следующая пустая, 0 в противном
               // случае, 2 конец файла (вызывать только когда с = \n)
int solver_default(int argc, char **argv);

int solver_b(FILE *fp);

int solver_e(FILE *fp);

int solver_E(FILE *fp);

int solver_n(FILE *fp);

int solver_s(FILE *fp);

int solver_t(FILE *fp);
...
int main(int argc, char **argv) {
  config options = command_line_dispatcher(argv);
...
config command_line_dispatcher(char **argv) {
  config options = {false, false, false, false, false, false,
