...
dispatcher_config init_dispatcher_config(command_line_options options,
                                         int count_files);
int find_index_start_files(
    int argc, char **argv); // функция для определения индекса аргумента с
                            // которого должно начинаться перечисление файлов
...
bool patterns_finder_in_str(char *str, command_line_options clo,
                            int cflag); // multi -e
dispatcher_config grep_dispatcher(
    command_line_options options,
    struct Node *files_storage_head); // add multi files / multi flags
...
int main(int argc, char **argv) {
  ...
  if (options.e == false && options.f == false) {
    add_Node(&options.pattern_storage,
             argv[find_index_start_files(argc, argv) - 1]);
  }
...
bool patterns_finder_in_str(char *str, command_line_options options,
                            int cflag) {
  bool result = false;
  for (int i = 0; i < get_size(options.pattern_storage); i++) {
    if (search_by_pattern(str, get_by_index(options.pattern_storage, i)->str,
                          cflag)) {
      result = true;
      break;
    }
...
command_line_options command_line_dispatcher(int argc, char **argv) {
  command_line_options options = {false, false, false, false, false, false,
...
dispatcher_config grep_dispatcher(command_line_options options,
                                  struct Node *files_storage_head) {
  bool flag_eof;
  ...
  while (counter_files < count_files) {
    ...
    config.str = (char *)malloc((str_size + 1) * sizeof(char));
    if (config.options.n) {
    ...
    while (!flag_eof) {
      ...
      if (!((!config.flag_v_helper && !options.v) ||
            (config.flag_v_helper && options.v))) {
        config.count_pattern_in_file++;
        output(config);
      }
      str_size = str_length(fp);
      free(config.str);
      config.str = (char *)calloc(str_size + 1, sizeof(char));
      iter++;
    }
